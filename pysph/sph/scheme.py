...
class ADKESchemeBoundary(Scheme):
    def __init__(self, fluids, solids, dim, rho0, p0, gamma=1.4, alpha=1.0,
                beta=2.0, k=1.0, eps=0.2, g1=0, g2=0, gx=0, gy=0, gz=0):
        self.fluids = fluids
        self.solids = solids
        self.dim = dim
        self.solver = None
    ...
    def get_equations(self):
        ...
        from pysph.sph.wc.transport_velocity import \
            SolidWallPressureBCWithoutDensity
        from pysph.sph.wc.transport_velocity import \
            SetWallVelocityReflect

        equations = []

        g2 = []
        for solid in self.solids:
            g2.append(WallBoundary(solid, sources=self.fluids))
        equations.append(Group(equations=g2))

        g1 = []
        for solid in self.solids:
            g1.append(SetWallVelocityReflect(dest=solid, sources=self.fluids))
        equations.append(Group(equations=g1))
        ...
        g5 = []
        for fluid in self.fluids:
            g5.append(SummationDensity(fluid, self.fluids+self.solids))
        equations.append(Group(g5))

        g6 = []
        for solid in self.solids:
            g6.append(WallBoundary(solid, sources=self.fluids))
        equations.append(Group(equations=g6))
        ...
        g7 = []
        for elem in self.fluids+self.solids:
            g7.append(IdealGasEOS(elem, sources=None, gamma=self.gamma))
        equations.append(Group(equations=g7))
        ...
        for fluid in self.fluids:
            g9.append(
                ADKEAccelerations(
                    dest=fluid, sources=self.fluids + self.solids,
                    alpha=self.alpha, beta=self.beta, g1=self.g1, g2=self.g2,
                    k=self.k, eps=self.eps, thrm_eps=2
                )
            )
        equations.append(Group(equations=g9))
    ...
    def setup_properties(self, particles, clean=True):
        ...
        required_props = [
                ...
                'e', 'au', 'av', 'aw', 'arho', 'ae', 'am', 'ah', 'x0', 'y0',
                'z0', 'u0', 'v0', 'w0', 'rho0', 'e0', 'h0', 'div',  'h0',
                'wij', 'htmp', 'logrho', 'wf', 'wg', 'vf', 'vg', 'uf', 'ug']
